...
export const JournalPage: React.FC = () => {
  ...
  const { data: transactions = [], isLoading, isError, error } = useTransactions({ date_debut: dateDebut, date_fin: dateFin, type: typeFiltre });

  if (isLoading) return <div className="p-6 text-center text-[var(--color-textSecondary)]">Chargement...</div>;

  // 🔧 Bug corrigé : avant, une erreur de chargement (RLS, réseau...)
  // affichait silencieusement "Aucune transaction enregistrée" (liste
  // vide par défaut), ce qui laissait croire à tort que le journal était
  // vide alors que la requête avait simplement échoué.
  if (isError) {
    return (
      <div className="p-6 max-w-7xl mx-auto">
        <div className="p-6 text-center text-[var(--color-danger)]">
          Erreur de chargement du journal{error instanceof Error ? ` : ${error.message}` : ''}
        </div>
      </div>
    );
  }
