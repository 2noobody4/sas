...
export const CompteResultatPage: React.FC = () => {
  ...
  const { data: resultat, isLoading } = useCompteResultat(dateDebut, dateFin);

  if (isLoading) return <div className="p-6 text-center text-[var(--color-textSecondary)]">Chargement...</div>;
  if (!resultat) return <div className="p-6 text-center text-[var(--color-danger)]">Erreur de chargement</div>;
