...
import { useFichesPaie, useCreateFichePaie, useUpdateFichePaie, useDeleteFichePaie, useAvancesSalaire } from '../hooks/useRH';
import { useEmployes, useContrats } from '../hooks/useRH';
import { Search, Plus, Trash2, FileText, X, CreditCard, Settings, Calendar, AlertCircle, DollarSign, Users, History, Wallet, CheckCircle2 } from 'lucide-react';
...
interface EmployePaieSummary {
  ...
  ficheId?: string;
  ficheStatut?: StatutFichePaie;
  ficheTransactionId?: string;
}
...
export const FichesPaiePage: React.FC = () => {
  ...
  // ✅ Mois courant calculé SANS side-effect
  ...
  const summaries: EmployePaieSummary[] = useMemo(() => {
    return employesDuMois.map((emp: any) => {
      ...
      const avancesData = getAvancesMois(emp.id, moisCourant);
      const prime = primeValues[emp.id] || 0;
      const ficheExistante = fiches.find((f: any) =>
        f.employe_id === emp.id && f.periode === moisCourant
      );
      ...
      return {
        ...
        ficheStatut: (ficheExistante as any)?.statut,
        ficheTransactionId: (ficheExistante as any)?.transaction_id,
      };
    });
  }, [employesDuMois, contrats, avances, fiches, primeValues, moisCourant]);
  ...
  // 🆕 Faire passer une fiche en "Validée" ou "Payée". C'est ce changement
  // de statut qui déclenche (une seule fois) la comptabilisation de la
  // charge de salaire — avant cette correction, aucune action de l'UI ne
  // permettait de sortir une fiche de son statut "brouillon" initial.
  const handleChangerStatut = async (ficheId: string, statut: StatutFichePaie) => {
    try {
      await updateMutation.mutateAsync({
        id: ficheId,
        statut,
        date_paiement: statut === 'payee' ? new Date().toISOString().split('T')[0] : undefined,
      });
      refetch();
    } catch (err: any) {
      toastError(err.message);
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--color-textSecondary)]">
            Gestion des paies du mois {moisCourant ? new Date(moisCourant).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' }) : ''}
          </p>
        </div>
                    ...
                    <tr key={emp.employeId} className="border-t border-[var(--color-borderColor)] hover:bg-[var(--color-secondary)]/50 transition">
                      ...
                      <td className="px-4 py-3 text-center">
                        {emp.ficheExistante ? (
                          <div className="flex flex-col items-center gap-1">
                            {getStatutBadge(emp.ficheStatut || 'brouillon')}
                            {(emp.ficheStatut === 'validee' || emp.ficheStatut === 'payee') && (
                              <span className={`text-[10px] ${emp.ficheTransactionId ? 'text-[var(--color-success)]' : 'text-[var(--color-warning)]'}`}>
                                {emp.ficheTransactionId ? '✅ comptabilisée' : '⚠️ non comptabilisée'}
                              </span>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex justify-center gap-1">
                          {emp.ficheExistante ? (
                            <>
                              {emp.ficheStatut === 'brouillon' && (
                                <button
                                  onClick={() => handleChangerStatut(emp.ficheId!, 'validee')}
                              ...
                              {emp.ficheStatut === 'validee' && (
                                <button
                                  onClick={() => handleChangerStatut(emp.ficheId!, 'payee')}
                                  disabled={updateMutation.isLoading}
                                  className="p-1.5 rounded hover:bg-green-50 text-[var(--color-success)]"
                                  title="Marquer payée (comptabilise le salaire)"
                                >
                                  <CreditCard size={16} />
                                </button>
                              )}
