...
import { useCreateProduit, useUpdateProduit, useProduit } from '../hooks/useProduits';
...
import { ArrowLeft, Plus, Scan, RefreshCw } from 'lucide-react';
...
export const ProductFormPage: React.FC = () => {
  ...
  const { data: produit, isLoading: produitLoading } = useProduit(id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      history.replace('/gestion/stocks/produits');
    } catch (err: any) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button
          onClick={() => history.replace('/gestion/stocks/produits')}
      ...
      <MotionBox type="card" variant="elevated" className="p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Nom */}
            <MotionBox as="div" type="box" variant="default" className="md:col-span-2">
            ...
            <MotionBox as="div" type="box" variant="default" className="md:col-span-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-xl bg-[var(--color-secondary)] text-[var(--color-textPrimary)] hover:bg-[var(--color-borderColor)] transition"
                >
                  <RefreshCw size={18} />
                </button>
                <button
            ...
            {/* Prix */}
            <MotionBox as="div" type="box" variant="default">
            ...
            <MotionBox as="div" type="box" variant="default">
              <label className="block text-sm font-medium text-[var(--color-textPrimary)]">Marge (%)</label>
              <input
                type="number"
                value={marge}
                onChange={(e) => {
            ...
            <MotionBox as="div" type="box" variant="default">
              <label className="block text-sm font-medium text-[var(--color-textPrimary)]">Bénéfice net (FCFA)</label>
              <input
                type="number"
                value={benefice}
                onChange={(e) => {
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-[var(--color-borderColor)]">
            ...
            >
              {isSubmitting ? (uploading ? 'Upload...' : 'Enregistrement...') : (isEdit ? 'Mettre à jour' : 'Créer')}
            </button>
          </div>
        </form>
      </MotionBox>
