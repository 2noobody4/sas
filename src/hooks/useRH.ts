    ...
    if ((data.statut === 'validee' || data.statut === 'payee') && userId && montantNet > 0) {
      try {
        ...
        if (compteChargePersonnel && compteBanque) {
          // 🔧 sourceTable: 'fiches_paie' — avant cette correction, le hook
          // générique mettait toujours à jour `saisies_comptables`, donc
          // `fiches_paie.transaction_id` n'était jamais renseigné (aucune
          // ligne à cet id n'existant dans `saisies_comptables`).
...
// ============================================================
// METTRE À JOUR UNE FICHE DE PAIE (validation / paiement)
// ------------------------------------------------------------
// 🆕 Hook manquant : jusqu'ici il n'existait AUCUN moyen de faire
// évoluer le statut d'une fiche de paie après sa création. Or
// `useCreateFichePaie` ne comptabilise QUE si le statut vaut déjà
// 'validee' ou 'payee' à la création — et `FichesPaiePage` créait
// systématiquement les fiches en 'brouillon'. Résultat : les charges
// de salaire n'étaient donc JAMAIS transmises à la comptabilité.
// Ce hook permet de valider/payer une fiche existante et déclenche,
// à cette occasion, la comptabilisation si elle n'a pas déjà eu lieu.
// ============================================================

export const useUpdateFichePaie = () => {
  const queryClient = useQueryClient();
  const { success, error: toastError } = useToast();
  const comptabiliserCharge = useComptabiliserCharge();
  ...
  const mutationFn = async ({
    ...
    id: string;
    statut: FichePaie['statut'];
    date_paiement?: string;
    // Comptes à utiliser si l'appelant souhaite les choisir explicitement.
    // À défaut, on retombe sur 641000 (charges de personnel) / 521000
    // (banque), comme à la création.
    ...
    if (!current) throw new Error('Fiche de paie introuvable');

    const { data: updated, error } = await supabase
      .from('fiches_paie')
      .update({
        statut,
        date_paiement: date_paiement || current.date_paiement || null,
    ...
    // 🔥 COMPTABILISATION à la validation/paiement — uniquement si la
    // fiche n'est pas déjà liée à une écriture comptable.
    const dejaComptabilisee = !!(current as any).transaction_id;
    if (!dejaComptabilisee && (statut === 'validee' || statut === 'payee') && userId && updated.montant_net > 0) {
      try {
        let compteChargePersonnelId = compteDebitId;
        let compteBanqueId = compteCreditId;

        if (!compteChargePersonnelId || !compteBanqueId) {
          const { data: comptes } = await supabase
            .from('comptes')
            .select('id, numero')
            .in('numero', ['641000', '521000']);
          compteChargePersonnelId = compteChargePersonnelId || comptes?.find((c: any) => c.numero === '641000')?.id;
          compteBanqueId = compteBanqueId || comptes?.find((c: any) => c.numero === '521000')?.id;
        }

        if (compteChargePersonnelId && compteBanqueId) {
          await comptabiliserCharge.mutateAsync({
            saisieId: id,
            montant: updated.montant_net,
            compteDebitId: compteChargePersonnelId,
            compteCreditId: compteBanqueId,
            type: 'salaire',
            libelle: `Salaire ${updated.periode} - Employé ${updated.employe?.user?.nom || ''}`,
            userId,
            sourceTable: 'fiches_paie',
          });
        } else {
          console.warn('[useUpdateFichePaie] Comptes 641000/521000 introuvables : initialisez le plan comptable OHADA.');
        }
      } catch (comptaError) {
        console.error('[useUpdateFichePaie] Erreur comptabilisation:', comptaError);
      }
    }
  ...
  return useOfflineMutation(
    {
      ...
      onSuccess: (_data, variables) => {
        ...
        queryClient.invalidateQueries({ queryKey: ['comptes'] });
        success(
          variables.statut === 'payee'
            ? 'Fiche de paie marquée payée ✅'
            : variables.statut === 'validee'
              ? 'Fiche de paie validée ✅'
              : 'Fiche de paie mise à jour ✅'
        );
      },
      onError: (err: any) => {
        toastError(err.message);
...
export const useDeleteFichePaie = () => {
  ...
  const mutationFn = async (id: string): Promise<string> => {
    // Supprimer les transactions comptables associées (contre-passation + delete)
    await annulerTransactionsParReference(id, ['salaire']);
