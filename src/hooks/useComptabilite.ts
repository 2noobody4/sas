    ...
    queryFn: async () => {
      try {
        let query = supabase
          ...
          .select('*, compte_debit:compte_debit_id(*), compte_credit:compte_credit_id(*), user:user_id(*)')
          .order('date_transaction', { ascending: false });

        if (filters?.date_debut) query = query.gte('date_transaction', filters.date_debut);
        if (filters?.date_fin) query = query.lte('date_transaction', filters.date_fin);
        ...
        const { data, error } = await query;
        if (error) throw error;
        return data as TransactionComptable[];
      } catch {
  ...
  return useQuery<Bilan, Error>({
    ...
    queryFn: async () => {
      ...
      comptes.forEach((c: Compte) => {
        const num = c.numero;
        if (num.startsWith('2')) actif.immobilisations += c.solde;
        else if (num.startsWith('3')) actif.stocks += c.solde;
        else if (num.startsWith('4')) {
          if (num.startsWith('41') || num.startsWith('42') || num.startsWith('44') || num.startsWith('46')) {
            actif.creances += c.solde;
          } else {
            passif.dettes_circulantes += c.solde;
          }
        } else if (num.startsWith('5')) actif.tresorerie += c.solde;
        else if (num.startsWith('1')) passif.capitaux_propres += c.solde;
        else if (num.startsWith('16')) passif.dettes_financieres += c.solde;
      });
...
export const useCompteResultat = (dateDebut?: string, dateFin?: string) => {
  return useQuery<CompteResultat, Error>({
    ...
    queryFn: async () => {
      ...
      if (!transactions) throw new Error('Transactions introuvables');
      ...
      transactions.forEach((t: TransactionComptable) => {
        ...
        if (debit && debit.numero.startsWith('6')) {
          ...
          else if (num.startsWith('64')) charges.impots += montant;
          else if (num.startsWith('66')) charges.personnel += montant;
