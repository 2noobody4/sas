...
export const useCreateVente = () => {
  ...
  const mutationFn = async (data: VenteFormData): Promise<Vente> => {
    ...
    // 2. Calculer le total et préparer les lignes
    let montantTotal = 0;
    const lignes: any[] = [];
    for (const ligne of data.lignes) {
      const { data: produit } = await supabase
        .from('produits')
        .select('quantite, prix_vente, magasin_id, entrepot_id')
        .eq('id', ligne.produit_id)
        .single();
      if (!produit) throw new Error(`Produit ${ligne.produit_id} introuvable`);
      if (produit.quantite < ligne.quantite) {
        throw new Error(`Stock insuffisant. Disponible: ${produit.quantite}`);
      }
    ...
    for (const ligne of data.lignes) {
      const { data: produitActuel } = await supabase
        .from('produits')
        .select('quantite, magasin_id, entrepot_id')
        .eq('id', ligne.produit_id)
        .single();
      if (produitActuel) {
