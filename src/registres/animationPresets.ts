...
import { AnimationParams } from '../types/theme-types';
import { LOOP_ANIMATIONS } from './animationsLoop';
...
export const animationsInitiales: Record<string, PresetEntree> = {
  ...
  // STAGGER pour GROUPES, LISTES, GRILLES
...
type AnimationParamsExtended = AnimationParams & {
  repeat?: number;
...
function applyParams(
  ...
  params?: AnimationParamsExtended
): Transition {
...
function parseAnimationRef(
  ref: string | { nom: string; params?: AnimationParams } | undefined
): { nom: string; params?: AnimationParamsExtended } {
  if (!ref) return { nom: '' };
...
export function resolveAnimation(animation?: {
  ...
  animation?.declenchees?.forEach((d) => {
    ...
    const mergedParams: AnimationParamsExtended = {
      ...(initiale?.params || {}),
      ...(parsed.params || {}),
    };
...
// ============================================================
// RÉSOLUTION D'ANIMATION EN BOUCLE
// ============================================================
export function resolveLoopAnimation(
  animationLoop?: string | { nom: string }
): { animate?: Record<string, any>; transition?: any } {
  if (!animationLoop) return {};

  const nom = typeof animationLoop === 'string' ? animationLoop : animationLoop.nom;
  if (!nom) return {};

  const preset = LOOP_ANIMATIONS[nom];
  if (!preset) {
    if (process.env.NODE_ENV === 'development') {
      console.warn(`[animationPresets] Loop inconnue: "${nom}"`);
    }
    return {};
  }
...
export const ANIMATION_PRESET_LIST = [
  ...Object.keys(animationsInitiales).map((id) => ({
    id,
    label: id,
    category: 'entree' as const,
    description: 'Animation d\'entrée',
  })),
  ...Object.keys(animationsSortie).map((id) => ({
    id,
    label: id,
    category: 'sortie' as const,
    description: 'Animation de sortie',
  })),
  ...Object.keys(animationsInteraction).map((id) => ({
    id,
    label: id,
    category: 'interaction' as const,
    description: 'Interaction',
  })),
  ...Object.keys(LOOP_ANIMATIONS).map((id) => ({
    id,
    label: LOOP_ANIMATIONS[id].label,
    category: 'loop' as const,
    description: LOOP_ANIMATIONS[id].description,
  })),
];
