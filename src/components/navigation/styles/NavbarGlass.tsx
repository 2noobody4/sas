// ============================================================
// NAVBAR GLASS — Style verre (transparent)
...
export const NavbarGlass: React.FC<NavbarGlassProps> = ({
  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 z-50 bg-white/10 backdrop-blur-xl border-t border-white/20 shadow-lg ${className}`}>
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] ${isActive ? 'text-white font-semibold' : 'text-white/70'}`}>
                {item.label}
              </span>
              {isActive && (
                <motion.div
