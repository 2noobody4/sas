...
import { Heart, Eye, ShoppingCart, Send } from 'lucide-react';
...
export const ProductCard: React.FC<ProductCardProps> = ({ produit, onCommander }) => {
  ...
  const ajouterAuPanierRapide = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!enStock) {
    ...
    >
      <div className="relative">
        ...
        <div className="absolute top-2 right-2 flex flex-col items-center gap-2 z-10">
          ...
          <button
            type="button"
            onClick={ajouterAuPanierRapide}
            disabled={!enStock}
            className={`p-2 rounded-full shadow-md backdrop-blur-sm transition ${
              enStock
                ? 'bg-white/85 text-[var(--color-textPrimary)] hover:bg-[var(--color-primary)] hover:text-white'
                : 'bg-white/60 text-[var(--color-textSecondary)] cursor-not-allowed opacity-60'
            }`}
