...
import { Upload, X, FileText, Plus, Eye } from 'lucide-react';
...
export const UploadDocument: React.FC<UploadDocumentProps> = ({
  ...
  // ✅ Renommer le paramètre en 'fileList' pour éviter le conflit
  const uploadFiles = async (fileList: FileList) => {
    ...
    try {
      for (const file of filesToUpload) {
        ...
        const fileName = `doc_${Date.now()}_${Math.random().toString(36).slice(2, 7)}.${fileExt}`;
        ...
        if (uploadError) {
          console.error('[UploadDocument] Erreur upload:', uploadError);
      ...
      if (newFiles.length > 0) {
        // ✅ Ici, 'files' est la prop du composant (string[]), pas le paramètre
        onChange([...files, ...newFiles]);
        success(`${newFiles.length} document${newFiles.length > 1 ? 's' : ''} uploadé${newFiles.length > 1 ? 's' : ''} ✅`);
      }
    } catch (err: any) {
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            {!isFull && !uploading && (
              ...
              >
                ...
                <span className="text-sm text-[var(--color-textSecondary)]">Ajouter un document</span>
              </MotionBox>
            )}
            {uploading && (
